...
const pkgDir = dirname(__dirname); // pi-candy-win-notify root
...
// Dest: where koffi expects it (inside pi-candy-win-notify/node_modules)
const destDir = join(pkgDir, "node_modules", "@koromix");
const dest = join(destDir, "koffi-win32-x64");

if (existsSync(dest)) {
  console.log("[pi-candy-win-notify] koffi native module already linked, skip");
  process.exit(0);
}

if (!existsSync(src)) {
  console.log("[pi-candy-win-notify] koffi native module not found at top level, skip");
  process.exit(0);
}

mkdirSync(destDir, { recursive: true });
execSync(`mklink /J "${dest}" "${src}"`, { stdio: "pipe" });
console.log("[pi-candy-win-notify] linked koffi native module");
