...
2. 选中一条消息后选择操作（菜单标题以竖线引用块显示所选消息与相对时间：`│ 「修复登录页…」` / `│ （5 分钟前）`）：
...
推荐把下面这条规则加进**全局规则**（`~/.pi/agent/AGENTS.md`）或项目 `AGENTS.md`，让模型默认走内置编辑工具：

```markdown
创建或修改文件必须使用内置的 `write` / `edit` 工具；严格禁止使用 Python、Node、`sed`、`echo >`、`cat <<EOF`、shell / bash / PowerShell / cmd / bat 等脚本或命令行方式创建或修改文件（这类改动无法被撤销插件跟踪）。
```

临时使用也可以走启动参数：`pi --append-system-prompt "<同样的规则>"`。
...
- 只跟踪 `write` / `edit`（可通过 `trackedTools` 扩展）；`bash` 与脚本造成的修改无法跟踪（可用全局规则强制模型只使用内置工具，见上文 [「让文件改动可被跟踪」](#让文件改动可被跟踪推荐)）
- **手动修改会被覆盖**：若某文件曾被 agent 改过（已跟踪），你之后手动改了它，`/undo` 会把它覆盖回目标时刻的内容（无脏检查，与 CC 一致）；确认菜单会先显示将改动的文件数与 ±行数，可改选"仅恢复对话"
- 把文件加入 `exclude` 只影响**后续**跟踪：之前已跟踪的文件仍会被回退
- 二进制文件按字节读写（安全），但 diff 统计按 1 行计
- 符号链接 / 硬链接路径不恢复也不删除（避免越界写入）
- 与工作区 git 完全无关，不产生任何 git 操作

> 各场景下文件如何变化的**完整矩阵**（哪些文件会被跟踪、回退时逐文件如何变化，以及手动修改 / `exclude` 规则 / 外部 git 操作等组合场景）见设计文档 [§3.1 文件变化矩阵](docs/design.md#31-文件变化矩阵跟踪与回退的完整行为)。
