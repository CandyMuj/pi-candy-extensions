...
```markdown
修改文件时优先使用内置的 `edit` / `write` 工具；除非确有必要，不要用 Python、Node、sed、
shell / bash / PowerShell / cmd / bat 等脚本或命令行方式修改文件
（这类改动无法被撤销插件跟踪）。
```
...
- 只跟踪 `write` / `edit`（可通过 `trackedTools` 扩展）；`bash` 与脚本造成的修改无法跟踪（可用全局规则约束模型优先用内置工具，见上文 [「让文件改动可被跟踪」](#让文件改动可被跟踪推荐)）
