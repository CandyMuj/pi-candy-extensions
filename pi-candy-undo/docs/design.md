...
- 只跟踪 agent 通过文件编辑工具改过的文件 → 手动改的、`bash` 改的文件天然不受影响（UI 提示与 CC 相同："Rewinding does not affect files edited manually or via bash"；完整的跟踪与回退行为矩阵见 §3.1）。
...
### 3.1 文件变化矩阵（跟踪与回退的完整行为）

**① 哪些文件会被跟踪**（其余文件一律不进入 `trackedFiles`，回退时完全不动）
...
**② 回退时对被跟踪文件逐一的处理**（`/undo` 的"仅恢复代码 / 恢复代码和对话"与 `/redo` 的代码部分共用同一套规则）

| 目标快照中的记录 | 当前文件状态 | 动作 |
|---|---|---|
| 有内容备份 | 内容/权限不同 | 写回备份内容并恢复权限位（自动创建缺失的父目录） |
| 有内容备份 | 内容一致 | 跳过（幂等，不计入"已恢复"） |
| 有内容备份 | 文件已不存在 | 重新创建（含父目录） |
| 记录为 `null`（该时刻不存在） | 存在 | 删除文件（不清理因此变空的目录） |
| 记录为 `null` | 已不存在 | 不动 |
| 无记录（该文件在该时刻尚未被跟踪） | — | 回落到 `originals`：有原始内容 → 恢复为"首次被 agent 编辑前"的内容；`originals` 为 `null`（文件由 agent 创建）→ 删除 |
| 符号链接 / 硬链接（`nlink > 1`） | — | 跳过，不写不删（确认菜单的统计里显示"N 个链接已跳过"） |
| 备份文件缺失或写入失败 | — | 记为该文件失败；**其余文件仍会继续处理**，全部处理完后如存在失败则 notify 错误（`notify.restoreFailed` 列出"文件 (原因)"、`notify.restoreFailedHint` 提示关闭占用程序）并中止本次回退（不导航、不入 redo 栈） |

**③ 容易踩的组合场景**

| 场景 | 结果 |
|---|---|
| 某文件曾被 agent 改过，之后**你手动改了它**，再 `/undo` | **会被覆盖**回目标时刻的内容（无脏检查，与 CC 一致）；确认菜单会先显示将改动的文件数与 ±行数，可改选"仅恢复对话" |
| 你手动改了**从未被 agent 改过**的文件，再 `/undo` | 不受影响（不在 `trackedFiles` 中） |
| agent 创建的文件，回退到创建之前 | 删除 |
| agent 改过的文件后来被删除（含 `bash rm`），回退到删除之前 | 重新创建 |
| 两次操作之间你手动改了已跟踪文件，之后又有 agent 操作 | 该手动改动会被**其后操作开始前的快照**捕获，回退到该操作之后可保留它 |
| 回退到已被 cap 淘汰的旧快照 | 回落到 `originals`（各文件"首次被编辑前"状态）或最近更早的存活快照，范围可能比预期大；确认菜单的统计会先显示 |
| 把某文件加入 `exclude` 之后 | **之前已跟踪的它仍会被回退**：排除规则只在跟踪时判定，恢复时不复查 |
| 文件被 `git checkout` 等外部手段改回旧版本 | 只要与目标备份内容不同，就会被覆盖为备份内容 |
| `storageDir` 内部的文件 | 永不跟踪、永不恢复 |
...
|---|---|
| 仅恢复对话 | 只回退对话，完全不碰文件 |
| 仅恢复代码 / 恢复代码和对话 | 恢复到 `/undo` 前自动建立的 redo-point 快照（撤销前一刻的文件状态），规则同 ② |
| redo 记录失效（快照已删或条目不存在） | 丢弃该记录并提示；恢复失败则 notify 错误并保留该记录以便重试 |
...
7. 执行：
   - code（或 both）：先建 redo-point 快照（当前 trackedFiles 状态，用于 /redo）
     ...
     → 恢复失败：notify 错误（列出失败文件与原因 + 提示关闭占用程序）并终止（不继续导航、不入 redo 栈）；逐文件进行，
       失败前已处理的文件保持已恢复状态（完整矩阵见 §3.1）
...
| 事项 | 措施 |
|---|---|
| 原子性 | 备份/state.json 均"临时文件 + rename"；恢复逐文件进行，单个文件失败不中断其余文件，全部处理完后如存在失败则 notify 错误（列出失败文件与原因 + 提示关闭占用程序）并中止本次回退（已处理的文件保持已恢复状态） |
