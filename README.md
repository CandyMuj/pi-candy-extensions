...
> **推荐 pi `>= 0.84.4`**（当前开发验证版本 `0.85.1`）
>
> - `0.84.4` 起官方新增 `ui_prompt_start` / `ui_prompt_end` 扩展事件，可精确识别"阻塞等待用户输入"（覆盖所有 `ctx.ui` 提问交互，如 ask、plan、permission 等插件），不再依赖插件名猜测
> - `pi-candy-win-notify` 的"等待用户"标签页状态在 `>= 0.84.4` 上自动走官方事件，识别更精确稳定；旧版回退为按 `waitingTools` 工具名匹配，对新插件/改名工具可能漏判
> - 建议 `0.84.3` 及以下版本升级后再使用本仓库插件
