...
- **每条列表项 = 一次用户可感知的变更**：默认把同一主题的多个提交（一个功能的 feat + 后续 fix/refactor + 配套 test/docs）合并为一条，描述由 AI 综合撰写；只涉及一个提交就只引用一个
- **纯内部维护提交不单独成条**（源码注释、文档措辞、死代码清理、纯测试补充等）：并入相关变更条目，或在本节末尾合并为一条「内部维护与文档」，引用全部相关 SHA
- **不跨插件/工具合并**；组内顺序以组内**最新提交**为准（保持新→旧）；触及多个插件目录的提交归「仓库公共」节
...
- **每条列表项末尾一律追加提交短 SHA**：`(<7位短hash>)`，例如 `(1a411bf)`；不加 `#`、不加反引号、不加链接；一条变更引用多个提交时并列：`(aaa1111, bbb2222)`
- **`#` 只用于 issue/PR 引用**（GitHub 语法会自动链接）
- 变更涉及 issue/PR 时：末尾括号内**短 SHA 在前、`#编号` 在后**，逗号分隔：`(1a411bf, #12)`；多个引用依次列出
- issue/PR 引用写在描述句内更自然时，写在句内，末尾只保留短 SHA
- 多条同主题提交可合并为一条（描述合成一句话），合并时在末尾**并列引用所有相关短 SHA**：`(aaa1111, bbb2222)`（合并是默认行为，见上）
