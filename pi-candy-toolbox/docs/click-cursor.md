...
- `debug`：开启后控制台打印鼠标事件、编辑器矩形查找、光标移动日志，便于排查点击无效问题（排查后建议关闭）

## 行为细节

- **左键按下**且落在编辑器矩形内 → 光标定位，事件被消费
- **其余一切事件**原样交给 pi 处理：滚轮滚动、右键粘贴、链接点击、编辑器外的点击、选区拖拽（编辑器外）——全部不受影响
- **已知限制**：从编辑器区域内开始拖拽的文本选区不再工作（按下被拦截为光标定位）；autocomplete 弹出列表的点击被忽略（不选中条目、不移动光标）
